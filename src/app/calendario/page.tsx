...
export default function CalendarioPage() {
    ...
    const { despesasReceitas, loading, error, refreshData } = useFinancialData(1);

    // Processar dados para calcular dias restantes (apenas despesas)
    const dadosProcessados = useMemo(() => {
        const hoje = new Date();

        return despesasReceitas
            .filter((item) => item.tipo === "despesa") // Apenas despesas
            .map((item) => {
