    ...
    useEffect(() => {
        async function fetchResumoFinanceiro() {
            try {
                ...
                const fetchAllData = async (endpoint: string) => {
                    ...
                    while (true) {
                        const response = await fetch(
                            `/api/${endpoint}?page=${page}&pageSize=${pageSize}&usuarioId=1&dataInicial=${dataInicial}&dataFinal=${dataFinal}`
                        );
                ...
                const totalEntradas = receitasData.reduce(
                    (total: number, receita: ReceitaSelect) => {
